#simulate parcel sampling and error (15% error) - this is for the Desk-based component which will use parcel based sampling
#Simon Gillings
#March 2016
...
#simulate samples of different size (50 to 10000 parcels) and determine the number that would be incorrect; repeate reps times
reps<-10000
nparcels<-seq(50,10000,by=10)
for(i in 1:length(nparcels)) {
  this.sample.size<-nparcels[i]
  event<-rbinom(reps,this.sample.size,0.15)
...
results$est.correct5<-ifelse(results$error.rate>=0.10 & results$error.rate<=0.20,100,0)
results$est.correct2<-ifelse(results$error.rate>=0.13 & results$error.rate<=0.17,100,0)
results$est.correct1<-ifelse(results$error.rate>=0.14 & results$error.rate<=0.16,100,0)
...
#produce graph
loc.name<-paste0('sim_parcel_random_sampling\\how_often_correctly_assessed_15pc.png')
