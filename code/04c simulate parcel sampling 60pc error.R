#simulate parcel sampling and error (60% error) - this is for the Desk-based component which will use parcel based sampling
#Simon Gillings
#March 2016
...
for(i in 1:length(nparcels)) {
  ...
  event<-rbinom(reps,this.sample.size,0.60)
...
results$est.correct5<-ifelse(results$error.rate>=0.55 & results$error.rate<=0.65,100,0)
results$est.correct2<-ifelse(results$error.rate>=0.58 & results$error.rate<=0.62,100,0)
results$est.correct1<-ifelse(results$error.rate>=0.59 & results$error.rate<=0.61,100,0)
...
#produce graph
loc.name<-paste0('sim_parcel_random_sampling\\how_often_correctly_assessed_60pc.png')
